...
import { parseRepoLink , createNewIndexer ,killIfCOntainerExists } from './helperFunctions';
import * as yaml from 'js-yaml';
import axios from 'axios';
var execSync = require('child_process').execSync;
const { apiKey, authDomain, projectId, storageBucket, messagingSenderId, appId } = require("./secrets.json")
...
userApp.post('/indexer/new' , jsonParser, async (req : Request, res: Response) => {
    try {
        let uid = await authVerify(req, res)
        if (uid == false) {
            return;
        }
        ...
        createNewIndexer(projid, user, repo, branch, RPC);
        ...
        await usercollection.updateOne({userId : uid}, {$push : {projs : projid}});
...
userApp.post('/indexer/update' , jsonParser, async (req : Request, res: Response) => {
    try {
        ...
        let ownership = await verifyOwnership(uid, projid, res);
        if (ownership == false) {
            return;
        }
